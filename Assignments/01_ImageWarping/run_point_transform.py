...
def upload_image(img):
    global image_input, points_src, points_dst
...
def record_points(evt: gr.SelectData):
    global points_src, points_dst, image_input
...
def point_guided_deformation(image, source_pts, target_pts, alpha=1.0, eps=1e-8):
    ...
    def affine_map(w, p_hat, p_star, v):
        try:
    ...
    def get_transformation_affine(p, v):
        ...
        p_star = centroid(w, p)
        p_hat = p - p_star
        matrix = affine_map(w, p_hat, p_star, v)
        def transformation(q):
            q_star = centroid(w, q)
            q_hat = q - q_star
            return (matrix * q_hat.transpose()).transpose().sum(axis=0) + q_star
        return transformation

    def similarity_rigid_map(w, p_hat, p_star, v):
        try:
            ...
            part1 = np.concatenate((p_hat,-p_hat_v), 0)
            part2 = np.concatenate((v_p_star,-v_p_star_v), 0).transpose()
            # w.reshape(1,1,-1) * np.einsum('pqi,qr', part1, part2).transpose(0,2,1)
            return w.reshape(1,1,-1) * np.tensordot(part1, part2, axes=(1,0)).transpose(0,2,1)
        except np.linalg.LinAlgError:
    ...
    def get_transformation_similarity(p, v):
        ...
        p_star = centroid(w, p)
        p_hat = p - p_star
        matrix = similarity_rigid_map(w, p_hat, p_star, v)
        mu_s = w @ (p_hat ** 2).sum(axis=1) + eps
        def transformation(q):
            q_star = centroid(w, q)
            q_hat = q - q_star
            # np.einsum('kj,jik', (q_hat, matrix / mu_s)) + q_star
            return np.tensordot(q_hat, matrix / mu_s, axes=([1, 0], [0, 2])) + q_star
        return transformation
    ...
    def get_transformation_rigid(p, v):
        ...
        def transformation(q):
            q_star = centroid(w, q)
            q_hat = q - q_star
            # np.einsum('kj,jik', (q_hat, matrix)) + q_star
            f_r = np.tensordot(q_hat, matrix, axes=([1, 0], [0, 2]))
...
def run_warping():
    global points_src, points_dst, image_input ### fetch global variables

    if len(points_dst) > 0:
        points_src_trimmed = points_src[:len(points_dst)]
        warped_image = point_guided_deformation(np.array(image_input), np.array(points_src_trimmed), np.array(points_dst))
        return warped_image
    else:
        return image_input
